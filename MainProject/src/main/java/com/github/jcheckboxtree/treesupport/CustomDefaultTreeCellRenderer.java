...
/**
 * CustomDefaultTreeCellRenderer, This is a customized copy of DefaultTreeCellRenderer.
 * (javax.swing.tree.DefaultTreeCellRenderer). The class was copied and customized so that we could
 * modify some needed private variables.
 *
 * Changes to the original class include:
 *
 * 1) Access and modifications to the focus border settings.
 *
 * ----------------------------------------------------------------------------------------------
 * The original DefaultTreeCellRenderer Javadocs are included below:
 * ----------------------------------------------------------------------------------------------
 * DefaultTreeCellRenderer, Displays an entry in a tree. <code>DefaultTreeCellRenderer</code> is not
 ...
 * <p>
 * The set of icons and colors used by {@code DefaultTreeCellRenderer} can be configured using the
 * various setter methods. The value for each property is initialized from the defaults table. When
 * the look and feel changes ({@code updateUI} is invoked), any properties that have a value of type
 * {@code UIResource} are refreshed from the defaults table. The following table lists the mapping
 * between {@code DefaultTreeCellRenderer} property and defaults table key:
 ...
 * This class overrides <code>invalidate</code>, <code>validate</code>, <code>revalidate</code>,
 * <code>repaint</code>, and <code>firePropertyChange</code> solely to improve performance. If not
 * overridden, these frequently called methods would execute code paths that are unnecessary for the
 * default tree cell renderer. If you write your own renderer, take care to weigh the benefits and
 ...
 * @author Scott Violet
 */
public class CustomDefaultTreeCellRenderer extends JLabel implements TreeCellRenderer {
    ...
    /**
     * If true, a dashed line is drawn as the focus indicator. (This has priority if
     * drawSolidFocusIndicator is also true).
     */
    public boolean drawDashedFocusIndicator;
    /**
     * If true, a solid line is drawn as the focus indicator.
     */
    public boolean drawSolidFocusIndicator;
    ...
    /**
     * Creates a {@code DefaultTreeCellRenderer}. Icons and text color are determined from the
     * {@code UIManager}.
     */
    public CustomDefaultTreeCellRenderer() {
        inited = true;
    }
    ...
    /**
     * Returns the default icon, for the current laf, that is used to represent non-leaf nodes that
     * are expanded.
     */
     ...
     * <code>ColorUIResource</code>, this has the effect of letting the background color of the
     * JTree show through. On the other hand, if <code>color</code> is non-null, and not a
